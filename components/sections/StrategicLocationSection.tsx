...
const HIGHWAY_KEYS = [
  { key: "highway57", descKey: "highway57Desc" },
  { key: "arcoNorte", descKey: "arcoNorteDesc" },
  { key: "circuito", descKey: "circuitoDesc" },
  { key: "rapidAccess", descKey: "rapidAccessDesc" },
] as const;

const ADVANTAGE_KEYS = [
  { title: "adv1Title", desc: "adv1Desc" },
  { title: "adv2Title", desc: "adv2Desc" },
  { title: "adv3Title", desc: "adv3Desc" },
  { title: "adv4Title", desc: "adv4Desc" },
  { title: "adv5Title", desc: "adv5Desc" },
  { title: "adv6Title", desc: "adv6Desc" },
] as const;

export function StrategicLocationSection() {
  const { language } = useLanguage();
  const t = translations[language].strategicLocation as Record<string, string>;
        ...
        <div className="relative bg-[#1F2937] rounded-3xl p-8 md:p-12 mb-16 overflow-hidden shadow-2xl">
          ...
          <div className="relative grid md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="text-xl   lg:text-2xl xl:text-3xl font-bold text-white mb-2">23M+</div>
              <div className="text-orange-100 font-medium">{t.consumers}</div>
            </div>
            <div className="text-center border-x border-gray-400">
              <div className="text-xl   lg:text-2xl xl:text-3xl font-bold text-white mb-2">5</div>
              <div className="text-orange-100 font-medium">{t.regions}</div>
            </div>
            <div className="text-center">
              <div className="text-xl   lg:text-2xl xl:text-3xl font-bold text-white mb-2">100%</div>
              <div className="text-orange-100 font-medium">{t.efficientNetwork}</div>
            </div>
          </div>
        </div>
